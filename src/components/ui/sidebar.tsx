      ...
      <SidebarContext.Provider value={contextValue}>
        <TooltipProvider delayDuration={0}>
          <div
            ...
            className={cn("group/sidebar-wrapper w-full", className)}
            ref={ref}
            {...props}
          >
            {children}
...
>(({ side = "left", className, children, ...props }, ref) => {
    ...
    return (
      <div
        ref={ref}
        data-state={state}
        className={cn("hidden lg:block shrink-0 transition-[width] duration-300 ease-in-out", 
            state === 'expanded' ? 'w-[var(--sidebar-width)]' : 'w-[var(--sidebar-width-icon)]',
            className
        )}
        {...props}
      >
        <div className="h-full w-full flex flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border">
          {children}
        </div>
      </div>
    )
...
>(({ className, ...props }, ref) => {
  return (
    <div
      ref={ref}
      className={cn("flex flex-col shrink-0 mt-auto", className)}
      {...props}
    />
  )
})
